...
namespace DistSSE{
		...
		static void generate_job_from_trace(Client* client, unsigned int thread_id, unsigned int step, std::atomic_size_t* entries_counter) {
			// unimplemented, generate db from existing trace

			return;
		}

		static void generate_job_from_trace(Client* client, unsigned int thread_id, size_t N_entries, unsigned int step, std::atomic_size_t* entries_counter) {
			// unimplemented, generate db from existing search trace, since update is determanistic

			return;
		}

		static void generation_trace(Client* client, unsigned int thread_id, size_t N_entries, unsigned int step, double search_rate, std::atomic_size_t* entries_counter) {
		...
		static void generation_simulation (Client* client, unsigned int thread_id, size_t N_entries, unsigned int step, double search_rate, std::atomic_size_t* entries_counter) {
			...
			std::unique_ptr<RPC::Stub> update_stub_(RPC::NewStub( grpc::CreateChannel("localhost:50051", grpc::InsecureChannelCredentials()) ) );

			std::unique_ptr<ClientWriterInterface<UpdateRequestMessage>> update_writer(update_stub_->batch_update(&update_context, &update_status));
			...
			std::unique_ptr<RPC::Stub> cache_stub_(RPC::NewStub( grpc::CreateChannel("localhost:50051", grpc::InsecureChannelCredentials()) ) );

			std::unique_ptr<ClientWriterInterface<CacheRequestMessage>> cache_writer(cache_stub_->batch_cache(&cache_context, &cache_status));
			...
			for(std::map<std::string,int>::iterator it = keyword_update_time.begin(); it != keyword_update_time.end(); ++it) {
				...
				for(size_t i = 0; i < update_after_search; i++) {
					...
					logger::log(logger::INFO) <<"keyword: "<< keyword<< " , update_after_search:" << update_after_search <<std::endl;		
				}// for
			...
			void gen_db(Client& client, size_t N_entries, unsigned int n_threads, double search_rate)
		    {
		        ...
		        for (unsigned int i = 0; i < n_threads; i++) {
		            threads.push_back(std::thread(generation_trace, &client, i, N_entries, n_threads, search_rate, &entries_counter));
		        }
		...
		void gen_simulation(Client& client, size_t N_entries, unsigned int n_threads, double search_rate)
		{
		        ...
		        for (unsigned int i = 0; i < n_threads; i++) {
		            threads.push_back(std::thread(generation_simulation, &client, i, N_entries, n_threads, search_rate, &entries_counter));
		        }
